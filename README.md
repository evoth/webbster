...
### 1. Download data from the MAST Portal
   - Follow the steps outlined in the [guide below](#downloading-data-from-the-mast-portal). After this is done, you should have a folder with only FITS files that end in `_i2d.fits`. Also check that `nircam` is somewhere in each filename, since as of now this project is tailored toward NIRCAM images from JWST.
...
### 2. Turn the raw images into viewable, aligned images with `fits-to-image.py`
   ...
   - For example, if your `.fits` files are in a folder called `fits`, and you want it to save the layers to a folder called `layers`, and you want the output image to be called `cosmic_cliffs.jpg`, then you would call `fits-to-image.py` as shown below:
      ...
      ```
   - If everything worked correctly, the layers folder should now have a monochrome image named something like `JW02731-F090W.png` for each `.fits` file you had, and the final image should also be ready to view!
...
### 3. Tweak the result using `combine-layers.py`
   ...
   - The result (`colors.txt` in this example) should look something like this:
      ...
      ```
      "layers/JW02731-F090W.png" (239, 87, 100)
      "layers/JW02731-F187N.png" (208, 100, 100)
      "layers/JW02731-F200W.png" (200, 67, 100)
      "layers/JW02731-F335M.png" (59, 75, 100)
      "layers/JW02731-F444W.png" (4, 24, 100)
      "layers/JW02731-F470N.png" (0, 98, 100)
      ```
...
4. In the text box for the `Instrument` filter (you may have to scroll down a little), type "`NIRCAM`" and hit enter.
5. Use the other filters to narrow down the search to your desired object:
   - The best way is to enter the object name into the `Object Name or Position` filter and press enter.
      - If you know an object's name (e.g. `Carina Nebula`) but not formal designation (e.g. `NGC 3372`), you can go its Wikipedia page and try the first designation listed under `Designations` in the sidebar on the right.
...
7. You should the results in a table (it may take some time to load).
...
4. For each folder containing the word `nircam`, open it and move the file ending in `_i2d.fits` to the folder created in step 2.
...
## Example images
| From `fits-to-image.py` | Tweaked using `combine-layers.py` | Colors file used |
| ----------------------- | --------------------------------- | ---------------- |
